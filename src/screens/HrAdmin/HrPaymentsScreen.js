...
import HrPaymentsListComponent from '../../components/HrAdmin/HrPaymentsListComponent';
...
export default class HrPaymentsScreen extends Component {
  constructor(props) {
    super(props);
    ...
    try {
      ...
      if (response) {
        ...
        if (success) {
          ...
          this.setState({payModes, isLoading: false, isRefreshing: false});
        } else {
  ...
  fetchPayments = async () => {
    const {fromDate, toDate, selectedModes} = this.state;
    try {
